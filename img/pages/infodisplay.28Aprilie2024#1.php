...
if ($ultimaInregistrare) {
    ...
    // Dacă nu există înregistrări, folosim data și ora curentă
    $timpUltimaModificare = date('Y-m-d H:i:s');
    // Setăm valori implicite pentru câmpuri
    $ultimaInregistrare = [
        ...
        'numeUnitateScolara' => '',
        'timp_ultima_modificare' => $timpUltimaModificare
    ];
}
...
</body> <!--aici se termina codul pentru cadranul-simulat ID4K-->
</html>

<script> /*codul javascript pentru cadranul-simulat ID4K*/
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  rezultate['lista_grupe_disponibile'].forEach(function(item) { /*optpional se poate selecta lista_clase_disponibile*/
    var optiune = document.createElement('option');
    optiune.text = item;
    optiune.value = item;
    select3.add(optiune);
