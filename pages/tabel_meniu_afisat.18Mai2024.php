...
<head>
    ...
    <style>
        ...
        th, td {
            ...
            padding: 2px; /* Reduce padding-ul pentru a micșora înălțimea celulelor */
            text-align: left;
            vertical-align: top; /* Asigură-te că conținutul se aliniază sus */
        }
        th {
        ...
        .inputText {
            font-size: 16px; /* Reduce dimensiunea fontului */
            line-height: 1.1; /* Reduce înălțimea liniei */
            width: 100%; /* Se ajustează la lățimea celulei */
            min-height: 20px; /* Înălțimea minimă inițială */
            ...
            box-sizing: border-box;
            padding: 2px; /* Adaugă padding pentru a face textul mai lizibil */
        }
    </style>
</head>
<body id="body_infodisplay">
    ...
    <table>
        ...
        <tr>
            <td class="coloana-ore"><textarea class="inputText oraInput">16:00</textarea></td>
...
document.getElementById('logo_infodisplay').addEventListener('click', function() {
    ...
    fetch('salveaza_meniuHTML.php', {
        ...
        body: 'html=' + encodeURIComponent(fullHTML) + '&id_utilizator=' + encodeURIComponent(JSON.stringify(<?= json_encode($id_utilizator); ?>))
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            console.log('HTML-ul meniului a fost salvat cu succes.');
            // Opcțional: Redirecționează către pagina avizierului
...
}); //sfarsitul algoritmului de confirmare editarii si salvare a meniuHTML

// Definește seturile de ore
var setOre1 = ["09:00", "10:30", "12:30", "16:00"];
// var setOre2 = ["08:15", "10:00", "12:00", "15:15", "19:00"];
