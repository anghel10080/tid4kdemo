...
function cadran_simulatValoriVariabile() {
    ...
    // Interogarea bazei de date pentru obținerea grupelor și claselor distincte
    $sql = "SELECT DISTINCT grupa_clasa_copil FROM copii";
    $result = mysqli_query($conn, $sql);

    // Procesare și filtrare pentru a obține lista_grupe_disponibile și lista_clase_disponibile
    $lista_grupe_disponibile = [];
    $lista_clase_disponibile = [];
    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
            if (strpos($row['grupa_clasa_copil'], 'grupa') !== false) {
                $lista_grupe_disponibile[] = $row['grupa_clasa_copil'];
            }
            if (strpos($row['grupa_clasa_copil'], 'clasa') !== false) {
                $lista_clase_disponibile[] = $row['grupa_clasa_copil'];
            }
        }
    }
    ...
    return [
        ...
        'lista_grupe_disponibile' => $lista_grupe_disponibile,
        'lista_clase_disponibile' => $lista_clase_disponibile,
         'surse_php' => [
...
$numeUnitateScolara = "Unitatea Scolara DEMO";
$cadran_simulatTitlu1Rand2 = "importantele...";
$cadran_simulatTitlu1Rand3 = "grupele...";
$cadran_simulatTitlu2Rand3 = "grupele...";
