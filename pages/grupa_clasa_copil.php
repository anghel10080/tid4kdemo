...
<?php
  ...
  <div class="container">
    <div class="column documente">
      ...
      <ul>
        <li class="doc-item">
        <a href="/pages/rapoarte_grupa_clasa_copil.php">Istoric prezență și contribuție</a>
        </li>
        <?php
...
<li class="doc-item">
    ...
    <a href="javascript:void(0);" onclick="showAnnouncementPopup();">anunțuri</a>
    <a href="/pages/documentele_noastre/documente_meniuri.php">și administrative</a>
</li>

<!-- Popup pentru anunțuri -->
<div class="popup-anunturi" id="popup-anunturi" style="display: none;">
    <div class="header-btn-anunturi">
        <button onclick="closeAnnouncementPopup()" class="close-btn-anunturi">Închide</button>
    </div>
    <div class="info-text">
        Anunturile sunt valabile 7 zile sau pana la data inserata direct in text, spre exemplu: "Mergem la Teatru in 20 Iunie", care va fi valabil pana in 20 Iunie, ora 23:59, anul curent.
    </div>
    <iframe id="announcement-iframe" src="introdu_anuntul.php" style="width: 100%; height: 400px;" frameborder="0"></iframe>
</div>

<script>
    function showAnnouncementPopup() {
        document.getElementById('popup-anunturi').style.display = 'block';
    }

    function closeAnnouncementPopup() {
        document.getElementById('popup-anunturi').style.display = 'none';
    }

    window.addEventListener('message', function(event) {
        if (event.data === 'announcementSaved') {
            // Afișăm bifa verde sau altă formă de confirmare
            alert('Anunțul este afișat la Avizier!'); // Poate fi înlocuit cu un mesaj vizual sau o bifă verde
            closeAnnouncementPopup(); // Închidem popup-ul
        }
    });
</script>
...
?>
        ...
        <li class="doc-item">
            <a href="javascript:void(0);" id="inscrierileButton">Înscrierile</a>
        </li>
<?php
   }
?>

<!-- Popup pentru Înscrierile -->
<div class="popup-inscrieri" id="popup-inscrieri">
    <div class="header-btn-inscrieri">
        <button onclick="printTableInscrieri()" class="print-btn-inscrieri">Print</button>
        <button onclick="closePopupInscrieri()" class="close-btn-inscrieri">Închide</button>
    </div>
    <h2 id="titlu-inscrieri"><span id="titlu-text"></span><span id="titlu-data"></span></h2>
    <table id="inscrieriTable">
        ...
        <tbody>
            <!-- Conținutul tabelului va fi completat dinamic -->
        </tbody>
    </table>
</div>

<script>
    document.getElementById('inscrierileButton').addEventListener('click', function() {
        fetch('tid4kinscrieri_api.php')
            .then(response => response.json())
            .then(data => {
                console.log('Received data:', data); // Debugging output
                populateTableInscrieri(data);
                showPopupInscrieri();
            })
            .catch(error => console.error('Error:', error));
    });

    function showPopupInscrieri() {
        // Setează titlul cu data și ora curente
        const now = new Date();
        const date = now.toLocaleDateString('ro-RO');
        const time = now.toLocaleTimeString('ro-RO', { hour: '2-digit', minute: '2-digit' });
        document.getElementById('titlu-text').textContent = 'Situația înscrierilor la data: ';
        document.getElementById('titlu-data').textContent = `${date} ${time}`;

        document.getElementById('popup-inscrieri').style.display = 'block';
    }

    function closePopupInscrieri() {
        document.getElementById('popup-inscrieri').style.display = 'none';
    }

    function populateTableInscrieri(data) {
        const tbody = document.getElementById('inscrieriTable').getElementsByTagName('tbody')[0];
        tbody.innerHTML = ''; // Clear existing content
        data.forEach(item => {
            const row = tbody.insertRow();
            row.insertCell(0).textContent = item.nume_prenume;
            row.insertCell(1).textContent = item.telefon;
            row.insertCell(2).textContent = item.data;
            row.insertCell(3).textContent = item.ora;
            const cell = row.insertCell(4);
            const link = document.createElement('a');
            link.href = 'tid4kinscrieri_download_zip.php?file=' + encodeURIComponent(item.fisier_url.split('/').pop());
                ...
                fetch(link.href)
                    ...
                    .then(downloadData => {
                        if (downloadData.status === 'success') {
                            link.style.color = 'green'; // Marcare descărcare
                            console.log('File downloaded:', downloadData.file); // Debugging output
                        } else {
    ...
    function printTableInscrieri() {
        const printContents = document.getElementById('inscrieriTable').outerHTML;
        const originalContents = document.body.innerHTML;
        document.body.innerHTML = printContents;
        window.print();
        document.body.innerHTML = originalContents;
        window.location.reload(); // Reîncarcă pagina pentru a restabili conținutul original
    }
</script>
    </ul>
    </div>
...
$(document).ready(function() {
   ...
   $('.nume-grupa').on('click', function() {
    ...
    console.log(`${tipElement} a fost DESCHISĂ`);
    ...
    if (!FereastraProfesoriSauCopiiDeschisa) {
        ...
        FereastraProfesoriSauCopiiDeschisa = false;
        console.log("Nume-grupa a fost INCHISĂ");
...
function contorizarePrezentaAbsenta(carduriCopii) {
    if (carduriCopii.length === 0) {
        ...
        console.log("Numărul de carduri de copii: ", carduriCopii.length);
    }
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            for (const mesaj of mesaje) {
                ...
                output += `
                <p>
                    ${mesaj.id_expeditor == id_utilizator ? `<span style="color: #2b516a;  font-size: 18px;">Dvs.</span>` : `<span style="color: #c32b28; cursive; font-size: 20px;">${mesaj.nume_expeditor}</span>`}
                    <span>${mesaj.mesaj}</span>
                    ${mesaj.id_destinatar == id_utilizator ? `<span style="color: #2b516a;  font-size: 18px;"> &#5139 Dvs.</span>` : `<span style="color: #c32b28; cursive; font-size: 20px;"> &#5139; ${mesaj.nume_destinatar}</span>`}
                </p>
                <p>
